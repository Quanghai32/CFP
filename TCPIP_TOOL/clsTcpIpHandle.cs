...
namespace TCPIP_TOOL
{
    public class clsTcpIpHandle
    {
        ...
        //For Client Role
        public string strTargetIPAddress = "";
        public int intTargetPort;
        ...
        public string SocketIniFileReading()
        {
            ...
            try
            {
                ...
                if (strTemp=="Error")
                {
                    MessageBox.Show("HostPort setting error. Auto config to default host port: 14000", "SocketIniFileReading() warning");
                    strTemp = "14000";
                }
                ...
                //Loading setting for Client role - target server setting
                //Get IP Address Config
                strTemp = MyLibrary.ReadFiles.IniReadValue("SOCKET_CLIENT", "TargetIpAddress", strSysSettingFullPath);
                if ((strTemp != "error") && (strTemp.Trim() != "")) //Config & config not empty
                {
                    this.strTargetIPAddress = strTemp;
                    this.strTargetIPAddress = this.strTargetIPAddress.Trim();
                }
                else //No config => turn to default is current setting of PC
                {
                    this.strTargetIPAddress = this.GetIPv4Address();
                }

                //Get Host Port Config
                strTemp = MyLibrary.ReadFiles.IniReadValue("SOCKET_CLIENT", "TargetPort", strSysSettingFullPath);
                if (strTemp == "Error")
                {
                    MessageBox.Show("TargetPort setting error. Auto config to default port: 14000", "SocketIniFileReading() warning");
                    strTemp = "14000";
                }

                if (int.TryParse(strTemp.Trim(), out this.intTargetPort) == false)
                {
                    MessageBox.Show("TargetPort setting error. Auto config to default port: 14000", "SocketIniFileReading() warning");
                    this.intTargetPort = 14000;
                }
            }
            catch (Exception ex)
            {
